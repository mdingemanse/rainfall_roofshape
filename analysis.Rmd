...
Here is an approach implmented first in 'The ecology of religious beliefs' by Carlos Botero et. al. This approach includes language family as a random effect and estimates the potential effects of spatial proximity through the average response value of nearest neighbours.

To estimate the effect of spatial proximity this approach includes the average socre among the N nearest neighbors for a given observation as a covariate in the model. We will look at this approach - however, since we are using a global sample, assuming a universal set of neighbours seems to ignore possible geographical distances (e.g. Rapanui's nearest 5 neighbours will be very different to a PNG society). So here we alter this approach to take the nearest neighbours within a certain distance - in this case 1000km.

Latitude and Longitude variables are used to calculate nearest neighbours. 
...
The first table here shows the delta AICc and AICc weight of each model. The second table shows the beta estimates for each variable in the model and the Relative Variable Importance (RVI). Predictive accuracy is another output from this model, but it hasn't been utilised here so is empty. 
...
Here, I have altered the approach slightly to take the average of neighbours within a certain distance. Currently I am just guesstimating the best distance, so a more methodological approach to choosing distance might be appropriate. I will start with neighbours within 1000kms. 
